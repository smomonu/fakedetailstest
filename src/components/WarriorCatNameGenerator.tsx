...
interface GeneratedWarriorCatName {
  id: string;
  name: string;
  category: string;
}

const WarriorCatNameGenerator: React.FC = () => {
  const [generatedNames, setGeneratedNames] = useState<GeneratedWarriorCatName[]>([]);
  ...
  const categories = React.useMemo(() => [
    ...
    { id: 'kit', name: 'Kit', icon: Sparkles },
    { id: 'elder', name: 'Elder', icon: Shield },
    { id: 'leader', name: 'Leader', icon: Zap },
    { id: 'medicine', name: 'Medicine Cat', icon: TestTube2 },
  ], []);

  const nameComponents = React.useMemo(() => ({
    warrior: {
      prefixes: ['Fire', 'Thunder', 'Lightning', 'Storm', 'Shadow', 'Raven', 'Hawk', 'Eagle', 'Wolf', 'Lion', 'Tiger', 'Leopard', 'Swift', 'Sharp', 'Bright', 'Dark', 'Silver', 'Golden', 'White', 'Black', 'Red', 'Blue', 'Gray', 'Brown', 'Amber', 'Frost', 'Ice', 'Snow', 'Stone', 'Rock'],
      suffixes: ['claw', 'fang', 'heart', 'spirit', 'tail', 'fur', 'eye', 'ear', 'paw', 'wing', 'storm', 'flame', 'blaze', 'strike', 'leap', 'dash', 'flight', 'song', 'whisper', 'roar', 'howl', 'cry', 'call', 'shine', 'glow', 'beam', 'spark', 'flash', 'bolt', 'runner']
    },
    apprentice: {
      prefixes: ['Fire', 'Thunder', 'Lightning', 'Storm', 'Shadow', 'Raven', 'Hawk', 'Eagle', 'Swift', 'Sharp', 'Bright', 'Dark', 'Silver', 'Golden', 'White', 'Black', 'Red', 'Blue', 'Gray', 'Brown', 'Amber', 'Frost', 'Ice', 'Snow', 'Stone', 'Rock', 'Leaf', 'Flower', 'Berry', 'Moss'],
      suffixes: ['paw']
    },
    kit: {
      prefixes: ['Fire', 'Thunder', 'Lightning', 'Storm', 'Shadow', 'Raven', 'Hawk', 'Eagle', 'Swift', 'Sharp', 'Bright', 'Dark', 'Silver', 'Golden', 'White', 'Black', 'Red', 'Blue', 'Gray', 'Brown', 'Amber', 'Frost', 'Ice', 'Snow', 'Stone', 'Rock', 'Leaf', 'Flower', 'Berry', 'Moss'],
    ...
    elder: {
      prefixes: ['Fire', 'Thunder', 'Lightning', 'Storm', 'Shadow', 'Raven', 'Hawk', 'Eagle', 'Wolf', 'Lion', 'Tiger', 'Leopard', 'Swift', 'Sharp', 'Bright', 'Dark', 'Silver', 'Golden', 'White', 'Black', 'Red', 'Blue', 'Gray', 'Brown', 'Amber', 'Frost', 'Ice', 'Snow', 'Stone', 'Rock'],
      suffixes: ['whisker', 'fur', 'tail', 'eye', 'ear', 'nose', 'tooth', 'claw', 'pelt', 'face', 'back', 'belly', 'leg', 'foot', 'step', 'walk', 'gaze', 'stare', 'breath', 'purr', 'mew', 'yowl', 'hiss', 'growl', 'snarl', 'snap', 'bite', 'scratch', 'swipe', 'strike']
    },
    leader: {
      prefixes: ['Fire', 'Thunder', 'Lightning', 'Storm', 'Shadow', 'Raven', 'Hawk', 'Eagle', 'Wolf', 'Lion', 'Tiger', 'Leopard', 'Swift', 'Sharp', 'Bright', 'Dark', 'Silver', 'Golden', 'White', 'Black', 'Red', 'Blue', 'Gray', 'Brown', 'Amber', 'Frost', 'Ice', 'Snow', 'Stone', 'Rock'],
      suffixes: ['star']
    ...
    medicine: {
      prefixes: ['Fire', 'Thunder', 'Lightning', 'Storm', 'Shadow', 'Raven', 'Hawk', 'Eagle', 'Swift', 'Sharp', 'Bright', 'Dark', 'Silver', 'Golden', 'White', 'Black', 'Red', 'Blue', 'Gray', 'Brown', 'Amber', 'Frost', 'Ice', 'Snow', 'Stone', 'Rock', 'Leaf', 'Flower', 'Berry', 'Moss', 'Herb', 'Root', 'Seed', 'Petal', 'Thorn'],
      suffixes: ['pool', 'stream', 'brook', 'creek', 'river', 'lake', 'pond', 'spring', 'mist', 'cloud', 'breeze', 'wind', 'gale', 'storm', 'rain', 'snow', 'frost', 'dew', 'dawn', 'dusk', 'moon', 'sun', 'light', 'shadow', 'spirit', 'soul', 'heart', 'mind', 'dream', 'vision']
    }
  }), []);
  ...
  const features = React.useMemo(() => [
    {
      ...
      description: 'Create long lists of warrior names instantly for clans, campaigns, and tests.'
    },
    {
      icon: Settings,
      title: 'Rank-aware Naming',
      description: 'Select rank to enforce rank-specific suffixes (‑paw, ‑kit, ‑star) automatically.'
    },
    {
      icon: Copy,
      title: 'One-Click Copy',
      description: 'Copy an individual name or bulk lists to clipboard with one action.'
    },
    {
      icon: Download,
      title: 'Export Options',
      description: 'Download .txt or export Markdown-ready lists for documentation and sharing.'
    },
    {
      icon: Shield,
      title: 'Local & Private',
      description: 'Generation runs entirely in your browser — nothing is uploaded or stored.'
    },
    {
      icon: Sparkles,
      title: 'Series-Accurate',
      description: 'Suffix patterns and prefixes are tuned to feel authentic to the Warriors tone.'
    }
  ], []);

  const useCases = React.useMemo(() => [
    { icon: Palette, title: 'Fan Fiction', description: 'Names for stories and lore.', color: 'bg-purple-500' },
    { icon: Gamepad2, title: 'Role-Playing', description: 'Seed characters for RPGs and sessions.', color: 'bg-blue-500' },
    { icon: Star, title: 'Character Sheets', description: 'Generate names for NPCs and player backstories.', color: 'bg-green-500' }
  ], []);

  const faqs = React.useMemo(() => [
    { question: 'Do names follow Warriors conventions?', answer: 'Yes — rank suffixes and patterns are applied to match the series tone.' },
    { question: 'Can I pick specific ranks?', answer: 'Yes — choose warrior, apprentice, kit, elder, leader, or medicine cat to force appropriate suffixes.' },
    { question: 'Are names usable in fan work?', answer: 'Yes — generated names are suitable for fan fiction and creative projects.' },
    { question: 'How many names can I generate?', answer: 'There\'s no enforced limit — generate a single name or thousands.' },
    { question: 'Is my data stored?', answer: 'No — generation runs locally in your browser.' }
  ], []);
  ...
  const generatedListRef = React.useRef<HTMLDivElement | null>(null);

  const generateNames = () => {
    const count = Math.max(1, batchCount);
    const newNames: GeneratedWarriorCatName[] = [];
    const usedNames = new Set<string>(generatedNames.map(n => n.name.toLowerCase()));

    // prevent infinite loops if prefix/suffix pool is too small
    ...
    while (newNames.length < count && attempts < maxAttempts) {
      ...
      if (!usedNames.has(lower)) {
        usedNames.add(lower);
        const id = (typeof crypto !== 'undefined' && (crypto as any).randomUUID) ? (crypto as any).randomUUID() : Date.now().toString() + Math.random().toString(36).slice(2, 9);
        newNames.push({ id, name, category: selectedCategory });
      }
    }

    if (newNames.length > 0) {
      setGeneratedNames(prev => [...newNames, ...prev]);
      // scroll to top of generated list to reveal newest names
      setTimeout(() => {
        generatedListRef.current?.scrollTo({ top: 0, behavior: 'smooth' });
      }, 50);
    }
  };
  ...
  const exportNames = () => {
    ...
    a.href = url;
    a.download = 'warrior-cat-names.txt';
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-lg text-white/80 max-w-2xl mx-auto leading-relaxed">
              Generate authentic, rank‑appropriate Warrior Cat names in seconds for fan fiction, role‑playing, and character creation. Customize suffixes, length, and style to get names that fit your clan’s voice.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 h-auto lg:h-[600px] flex flex-col">
                <h3 className="text-xl font-semibold text-gray-900 mb-6">Generated Warrior Cat Names</h3>
                
                <div ref={generatedListRef} className="flex-1 flex flex-col lg:min-h-0">
                  {generatedNames.length === 0 ? (
                    <div className="text-center py-8 lg:py-12 flex-1 flex flex-col justify-center">
                      <Shield className="w-12 h-12 lg:w-16 lg:h-16 mx-auto mb-4 text-gray-400" />
                      <p className="text-base lg:text-lg text-gray-500">No Warrior Cat names generated yet</p>
      ...
      <section id="how-it-works" className="py-12 md:py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-base md:text-xl text-gray-600 max-w-3xl mx-auto">Generate rank-appropriate Warrior Cat names quickly: pick a rank, tune options, generate, then export.</p>
          </div>
          ...
          <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-8 items-start">
            <div className="text-center">
              ...
              <Shield className="w-6 h-6 text-purple-600 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-gray-900 mb-3">Select Rank</h3>
              <p className="text-gray-600 leading-relaxed">Choose Warrior, Apprentice, Kit, Elder, Leader, or Medicine Cat to apply rank-aware suffixes.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 leading-relaxed">Adjust length, include numbers or symbols, and prefer compound vs single-word formats.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 leading-relaxed">Create names in bulk for clans, NPCs, or role-playing campaigns.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 leading-relaxed">Download plain-text lists or copy names for sharing and documentation.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-12 md:py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-base md:text-xl text-gray-600 max-w-3xl mx-auto">Rank-aware suffixes, bulk exports, and privacy-first generation for fans and creators.</p>
          </div>
      ...
      <section id="why-choose-us" className="py-12 md:py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-base md:text-xl text-gray-600 max-w-3xl mx-auto">Create authentic Warrior Cat names quickly with patterns tuned for fan works and RPGs.</p>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="bg-gradient-to-br from-white to-gray-50 p-8 rounded-xl shadow-sm hover:shadow-xl border border-gray-100">
              ...
              <h3 className="text-2xl font-semibold text-gray-900 mb-4">Series-Accurate Patterns</h3>
              <p className="text-gray-600 leading-relaxed">Suffix rules and prefixes are tuned to feel true to the Warriors universe while remaining original.</p>
            </div>
            <div className="bg-gradient-to-br from-white to-gray-50 p-8 rounded-xl shadow-sm hover:shadow-xl border border-gray-100">
              <div className="w-16 h-16 bg-indigo-500 rounded-xl flex items-center justify-center mb-6">
                <Palette className="w-8 h-8 text-white" />
              </div>
              <h3 className="text-2xl font-semibold text-gray-900 mb-4">Customizable Ranks</h3>
              <p className="text-gray-600 leading-relaxed">Enforce kit/apprentice/warrior suffixes or generate cross-rank variants for storytelling.</p>
            </div>
            <div className="bg-gradient-to-br from-white to-gray-50 p-8 rounded-xl shadow-sm hover:shadow-xl border border-gray-100">
              ...
              <p className="text-gray-600 leading-relaxed">Download ready lists for wikis, campaigns, and collaborative projects.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-gradient-to-r from-purple-600 to-blue-600">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold text-white mb-6">Start naming your clan</h2>
          <p className="text-xl text-purple-100 mb-8 max-w-2xl mx-auto">Generate Warrior Cat names for stories, RPGs, and fan projects — fast, private, and export-ready.</p>
