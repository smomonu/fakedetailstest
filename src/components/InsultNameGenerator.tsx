...
import { 
  ...
  Menu,
  X,
  Star,Gamepad2,Sparkles,CheckCircle,Smile,
  Zap, Code
} from 'lucide-react';

interface GeneratedInsult {
  id: string;
  insult: string;
  category: string;
}

const InsultNameGenerator: React.FC = () => {
  const [generatedInsults, setGeneratedInsults] = useState<GeneratedInsult[]>([]);
  const [selectedCategory, setSelectedCategory] = useState<string>('mild');
  ...
  const categories = [
    { id: 'mild', name: 'Mild & Playful', icon: Smile },
    { id: 'creative', name: 'Creative & Witty', icon: Sparkles },
    { id: 'medieval', name: 'Medieval Style', icon: Shield },
    { id: 'shakespearean', name: 'Shakespearean', icon: Palette },
    { id: 'silly', name: 'Silly & Funny', icon: Star },
    { id: 'nerdy', name: 'Nerdy & Geeky', icon: TestTube2 },
  ];

  const insultComponents = {
    mild: {
      adjectives: ['silly', 'goofy', 'clumsy', 'forgetful', 'absent-minded', 'scatterbrained', 'ditzy', 'dopey', 'bumbling', 'befuddled'],
      nouns: ['goose', 'noodle', 'muffin', 'pickle', 'turnip', 'potato', 'banana', 'cookie', 'marshmallow', 'jellybean']
    },
    creative: {
      adjectives: ['intellectually-challenged', 'wisdom-impaired', 'logic-deficient', 'brain-absent', 'thought-lacking', 'sense-deprived', 'wit-wanting', 'reason-resistant', 'understanding-challenged', 'comprehension-free'],
      nouns: ['individual', 'specimen', 'example', 'case study', 'phenomenon', 'curiosity', 'marvel', 'wonder', 'enigma', 'mystery']
    },
    medieval: {
      adjectives: ['daft', 'addled', 'witless', 'dim-witted', 'dull', 'slow', 'thick-skulled', 'empty-headed', 'bird-brained', 'half-witted'],
      nouns: ['knave', 'varlet', 'cur', 'scoundrel', 'rogue', 'wretch', 'fool', 'jester', 'buffoon', 'peasant']
    },
    shakespearean: {
      adjectives: ['artless', 'bawdy', 'beslubbering', 'bootless', 'churlish', 'cockered', 'clouted', 'craven', 'currish', 'dankish'],
      nouns: ['apple-john', 'baggage', 'barnacle', 'bladder', 'boar-pig', 'bugbear', 'bum-bailey', 'canker-blossom', 'clack-dish', 'clotpole']
    },
    silly: {
      adjectives: ['wobbly', 'giggly', 'wiggly', 'jiggly', 'squiggly', 'bubbly', 'gibbering', 'blabbering', 'dribbling', 'babbling'],
      nouns: ['nincompoop', 'doofus', 'goofball', 'knucklehead', 'blockhead', 'numbskull', 'dimwit', 'nitwit', 'halfwit', 'peabrain']
    },
    nerdy: {
      adjectives: ['404-brained', 'lag-prone', 'buffering', 'glitchy', 'buggy', 'crashed', 'frozen', 'corrupted', 'outdated', 'deprecated'],
      nouns: ['noob', 'script kiddie', 'user', 'NPC', 'bot', 'troll', 'lurker', 'newbie', 'casual', 'normie']
    }
  };
  ...
  const features = [
    {
      ...
      description: 'Produce large lists of clever insults instantly — ideal for writers, streamers, and roast sessions.'
    },
    {
      icon: Settings,
      title: 'Style Presets',
      description: 'Pick a tone (mild, medieval, Shakespearean, nerdy, silly) to guide word choice and cadence.'
    },
    {
      icon: Copy,
      title: 'One-Click Copy',
      description: 'Copy single lines or whole batches to clipboard with a single action for fast reuse.'
    },
    {
      icon: Download,
      title: 'Export & Formats',
      description: 'Download plain-text lists or Markdown-ready output for docs, scripts, or social posts.'
    },
    {
      icon: Shield,
      title: 'Local & Private',
      description: 'All generation runs in your browser so nothing is uploaded or stored externally.'
    },
    {
      icon: Sparkles,
      title: 'Tone-Tuned',
      description: 'Suffixes and phrasing are tuned to fit each style so results feel natural and varied.'
    }
  ];
  ...
  const useCases = [
    {
      ...
      description: 'Generate witty insults for comedy sketches, stand-up routines, and humorous creative writing projects.',
      color: 'bg-purple-500'
    },
    {
      icon: Gamepad2,
      title: 'Gaming & Roasts',
      description: 'Create playful banter for gaming sessions, friendly roasts, and competitive trash talk.',
      color: 'bg-blue-500'
    },
    {
      icon: Sparkles,
      title: 'Entertainment',
      description: 'Perfect for social media content, memes, and light-hearted entertainment purposes.',
      color: 'bg-green-500'
    }
  ];
  ...
  const faqs = [
    {
      question: 'Are these insults safe to use?',
      answer: 'They are crafted to be playful rather than harmful, but please review before using in sensitive or professional contexts.'
    },
    {
      question: 'Can I use the results commercially?',
      answer: 'Yes — generated lines may be used in comedy, games, social content, and other creative work.'
    },
    {
      question: 'Why choose Shakespearean style?',
      answer: 'The Shakespearean set uses Elizabethan-era wording and rhythms to create authentic period-flavored jabs.'
    },
    {
      question: 'How many can I generate?',
      answer: 'You can create a single line or bulk lists — there is no enforced limit.'
    },
    {
      question: 'Is my data private?',
      answer: 'Yes. Everything runs locally in your browser; nothing is uploaded or stored on our servers.'
    }
  ];

  const generateSingleInsult = (): string => {
    const category = insultComponents[selectedCategory as keyof typeof insultComponents];
    const adjective = category.adjectives[Math.floor(Math.random() * category.adjectives.length)];
    const noun = category.nouns[Math.floor(Math.random() * category.nouns.length)];
    
    // Different formats based on category
    if (selectedCategory === 'creative') {
      return `You ${adjective} ${noun}`;
    } else if (selectedCategory === 'shakespearean') {
      return `Thou ${adjective} ${noun}`;
    } else {
      return `You ${adjective} ${noun}`;
    }
  };

  const generateInsults = () => {
    const count = Math.max(1, batchCount);
    const newInsults: GeneratedInsult[] = [];
    const usedInsults = new Set();
    
    while (newInsults.length < count) {
      const insult = generateSingleInsult();
      
      if (!usedInsults.has(insult.toLowerCase())) {
        usedInsults.add(insult.toLowerCase());
        newInsults.push({
    ...
    setGeneratedInsults(prev => [...newInsults, ...prev]);
  };
  ...
  const exportInsults = () => {
    const text = generatedInsults.map(i => i.insult).join('\n');
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'insults.txt';
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-lg text-white/80 max-w-2xl mx-auto leading-relaxed">
              Create original, tone-aware jabs instantly — tailored for comedy sketches, friendly roasts, and punchy social posts. Pick a voice, adjust details, and export lists you can use right away.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 h-auto lg:h-[600px] flex flex-col">
                <h3 className="text-xl font-semibold text-gray-900 mb-4">Insult Settings</h3>
                ...
                <div className="space-y-3 border-t border-gray-200 pt-3 bg-white flex-shrink-0">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="25"
                    />
                    <p className="text-xs text-gray-500 mt-1 text-center">Generate unlimited insults at once</p>
                  </div>
                  ...
                  >
                    ...
                    <span>Generate Insults</span>
                  </button>
                  
                  {generatedInsults.length > 0 && (
                    <div className="pt-2 border-t border-gray-200">
                      <div className="text-sm text-gray-600 mb-2 text-center">
                        {generatedInsults.length} insult{generatedInsults.length !== 1 ? 's' : ''} generated
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 sm:space-x-2">
                    <button
                      onClick={exportInsults}
                      disabled={generatedInsults.length === 0}
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 h-auto lg:h-[600px] flex flex-col">
                <h3 className="text-xl font-semibold text-gray-900 mb-6">Generated Creative Insults</h3>
                
                <div className="flex-1 flex flex-col lg:min-h-0">
                  {generatedInsults.length === 0 ? (
                    <div className="text-center py-8 lg:py-12 flex-1 flex flex-col justify-center">
                      <Zap className="w-12 h-12 lg:w-16 lg:h-16 mx-auto mb-4 text-gray-400" />
                      <p className="text-base lg:text-lg text-gray-500">No insults generated yet</p>
                      <p className="text-sm lg:text-base text-gray-400">Set how many lines you want, choose a style, then click "Generate Insults" to produce a fresh batch.</p>
                    </div>
                  ) : (
                    <div className="flex-1 flex flex-col lg:min-h-0">
                      <div className={`space-y-3 flex-1 lg:min-h-0 ${generatedInsults.length > 9 ? 'lg:overflow-y-auto' : ''}`}>
                        {generatedInsults.map((item) => (
                          <div
                            key={item.id}
                          ...
                          >
                            <div className="flex items-center space-x-3">
                              ...
                              <div className="min-w-0 flex-1">
                                <p className="font-semibold text-gray-900 text-base sm:text-lg break-all">{item.insult}</p>
                                <p className="text-xs sm:text-sm text-gray-500 capitalize">{item.category} style</p>
                              </div>
                            </div>
                            <button
                              onClick={() => copyToClipboard(item.insult, item.id)}
      ...
      <section id="how-it-works" className="py-12 md:py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-base md:text-xl text-gray-600 max-w-3xl mx-auto">Create playful, tone-aware insults quickly: pick a voice, adjust options, generate lines, then export.</p>
          </div>
          ...
          <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-8 items-start">
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-3">Pick a Voice</h3>
              <p className="text-gray-600 leading-relaxed">Choose Mild, Creative, Medieval, Shakespearean, Silly, or Nerdy to set tone and vocabulary.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 leading-relaxed">Adjust length, include numbers or symbols, and choose single-line or multi-line formats.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 leading-relaxed">Produce single lines or bulk batches suitable for sketches, streams, or playful roasts.</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 leading-relaxed">Download .txt or Markdown lists, or copy lines to clipboard for immediate use.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-12 md:py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-base md:text-xl text-gray-600 max-w-3xl mx-auto">Tone presets, rapid bulk output, and safe, local generation so creators can iterate fast.</p>
          </div>
      ...
      <section id="why-choose-us" className="py-12 md:py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-base md:text-xl text-gray-600 max-w-3xl mx-auto">Fast iterations, curated tones, and export-ready outputs for writers, streamers, and creators.</p>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="bg-gradient-to-br from-white to-gray-50 p-8 rounded-xl shadow-sm hover:shadow-xl border border-gray-100">
              ...
              <h3 className="text-2xl font-semibold text-gray-900 mb-4">Tone-Tuned</h3>
              <p className="text-gray-600 leading-relaxed">Presets and curated components produce natural-sounding insults appropriate to the selected voice.</p>
            </div>
            <div className="bg-gradient-to-br from-white to-gray-50 p-8 rounded-xl shadow-sm hover:shadow-xl border border-gray-100">
              ...
              <p className="text-gray-600 leading-relaxed">Control length, symbols, and batch size to suit streaming cadence, scripts, or posts.</p>
            </div>
            <div className="bg-gradient-to-br from-white to-gray-50 p-8 rounded-xl shadow-sm hover:shadow-xl border border-gray-100">
              ...
              <p className="text-gray-600 leading-relaxed">Export plain-text or Markdown-ready lists for immediate use in production and publishing.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-gradient-to-r from-purple-600 to-blue-600">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold text-white mb-6">Start generating lines</h2>
          <p className="text-xl text-purple-100 mb-8 max-w-2xl mx-auto">Create playful insults for comedy, streams, and social posts — fast, private, and export-ready.</p>
...
export default InsultNameGenerator; 